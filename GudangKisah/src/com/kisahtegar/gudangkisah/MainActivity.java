...
public class MainActivity extends Activity {
	...
	private void getData() {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                String urlString = "http://192.168.0.15:8081/gudangkisah/getdata.php";
                try {
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        String id_barang = jsonObject.getString("id_barang");
                        String nama = jsonObject.getString("nama");
                        String jenis = jsonObject.getString("jenis");
                        String stock = jsonObject.getString("stock");

                        String item = "ID Barang: " + id_barang + "\nNama: " + nama + "\nJenis: " + jenis + "\nStock: " + stock;
                        barangList.add(item);
                    }
	...
	private void deleteData(final String id_barang, final int position) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String urlString = "http://192.168.0.15:8081/gudangkisah/hapus.php";
                try {
                    URL url = new URL(urlString);
                    HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
